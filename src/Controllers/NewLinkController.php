...
class NewLinkController extends BaseController
{
    // 请求入口
    public function requestEntry($request, $response, $args)
    {
        $token = $args['token'];
        $user_agent = $_SERVER['HTTP_USER_AGENT'];
        $exist = Link::where('token', $token)->first();
        if ($exist === null) {
            return;
        }
        $user = $exist->getUser();
        ...
        switch ($client) {
            case 'clash':
                return self::clashClient($nodes, $user);
            default:
                return self::v2raynClient($nodes, $user->uuid);
        }
    }

    // 检测订阅客户端
    public static function detectTheSubscribingClient(string $ua): string
    {
        if (strpos($ua, 'Clash') !== false) {
            return 'clash';
        }
    ...
    public static function v2raynClient(object $nodes, string $user_uuid): string
    {
        ...
        foreach ($nodes as $node) {
            switch ($node->parsing_mode) {
                case 'v2ray_ws':
                case 'v2ray_ws_tls':
                    $is_tls = ($node->parsing_mode === 'v2ray_ws') ? false : true;
    ...
    // 服务clash客户端订阅 最后返回经yaml编码的数据
    public static function clashClient(object $nodes, object $user): string
    {
        $node_configs = [];
        $node_names = self::getAllNodeNamesForTheClashClient($nodes);
        ...
        foreach ($nodes as $node) {
            switch ($node->parsing_mode) {
                ...
                case 'v2ray_ws_tls':
                    $is_tls = ($node->parsing_mode === 'v2ray_ws') ? false : true;
                    $group_nodes = self::parseClashV2rayWebSocket($node, $user->uuid, $is_tls);
                    break;
                case 'trojan_grpc':
                    $group_nodes = self::parseClashTrojanGrpc($node, $user->uuid);
                    break;
            }
            // 剥离出来
            foreach ($group_nodes as $single_node) {
                $node_configs[] = $single_node;
            }
        }

        // https://docs.cfw.lbyczf.com/contents/urlscheme.html#%E5%93%8D%E5%BA%94%E5%A4%B4
        $wait_encode = self::clashClientTemplate($node_configs, $node_names); // array
    ...
    // 解析额外参数
    // 传入 key1=value1|key2=value2 这样的字符串
    // 传出 key => value 的关联数组
    ...
    // 为clash客户端订阅服务 传出所有节点名称(包含单节点多入口的配置)数组
    public static function getAllNodeNamesForTheClashClient(object $nodes): array
    {
        $names = [];
    ...
    public static function parseV2rayWebSocket(object $node, string $user_uuid, bool $is_tls): string
    {
        $text = '';
        ...
        if ($node->add_in === 1) {
            // https://www.jianshu.com/p/91fe5a065a5b
    ...
    public static function parseTrojanGrpc(object $node, string $user_uuid): string
    {
        $text = '';
        $split = explode(';', $node->server);
        $params = self::parsingAdditionalParameters($split[1]);
        if ($node->add_in === 1) {
            $addr = $split[0];
            $port = $params['port'];
            $path = $params['servicename'];
            $name = rawurlencode($node->name);
            $text .= "trojan://{$user_uuid}@{$addr}:{$port}?peer={$addr}&sni={$addr}&obfs=grpc&path={$path}&obfsParam={$addr}&type=grpc&security=tls&serviceName={$path}#{$name}" . PHP_EOL;
        }
        if ($node->transit_enable === 1) {
            $configs = json_decode($node->transit_json, true);
            foreach ($configs as $config) {
                ...
                $addr = $config['sni_instruction'];
                $connect_addr = $config['connect_addr'];
                $name = rawurlencode($config['display_name']);
                $text .= "trojan://{$user_uuid}@{$connect_addr}:{$port}?peer={$addr}&sni={$addr}&obfs=grpc&path={$path}&obfsParam={$addr}&type=grpc&security=tls&serviceName={$path}#{$name}" . PHP_EOL;
            }
        }
    ...
    public static function parseClashV2rayWebSocket(object $node, string $user_uuid, bool $is_tls): array
    {
        $all = [];
        $split = explode(';', $node->server);
        $params = self::parsingAdditionalParameters($split[5]);
        ...
        if ($node->transit_enable === 1) {
            ...
            foreach ($configs as $config) {
                // 覆写配置
                $array['name'] = $config['display_name'];
                $array['port'] = $config['connect_port'];
                $array['server'] = $config['connect_addr'];
                $array['servername'] = $config['sni_instruction'];
                $all[] = $array;
            }
        }
    ...
    public static function parseClashTrojanGrpc(object $node, string $user_uuid): array
    {
        $all = [];
        $split = explode(';', $node->server);
        $params = self::parsingAdditionalParameters($split[1]);
    ...
    public static function LoggingSubscriptions($user, $type, $ua)
    {
        ...
        $log->request_user_agent = $xss->xss_clean($ua);
        $log->save();
    }

    public static function clashClientTemplate(array $node_configs, array $node_names): array
    {
        $array = [
            'port' => 7890,
            'socks-port' => 7891,
            ...
            'rules' => [
                ...
                'DOMAIN,clientservices.googleapis.com,🔰 节点选择',
                'DOMAIN,dl.google.com,🔰 节点选择',
                'DOMAIN,dl.l.google.com,🔰 节点选择',
                'DOMAIN,update.googleapis.com,🔰 节点选择',
                'DOMAIN,translate.googleapis.com,🔰 节点选择',
                'DOMAIN,translate.google.com,🔰 节点选择',
                'DOMAIN,translate.google.cn,🔰 节点选择',
                'DOMAIN,fonts.googleapis.com,🔰 节点选择',
                'DOMAIN,fonts.gstatic.com,🔰 节点选择',
